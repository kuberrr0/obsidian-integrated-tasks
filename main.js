...
function noteRecurringCompletion(getDateFormat, isRecurring, complete2, getPath = (state) => {
  ...
  return [
    ...
    import_view.EditorView.updateListener.of((update) => {
      const tasks = update.transactions.flatMap((transaction) => transaction.effects.filter((effect) => effect.is(recurringCompletion)).flatMap((effect) => effect.value));
      if (tasks.length) void Promise.resolve().then(() => tasks.forEach((task) => complete2(task)));
    })
  ];
}
...
function appendRecurringLog(content, outcome, date, dateFormat = "YYYY-MM-DD", linkDates = false) {
...
// src/project-creator.ts
var ProjectCreatorModal = class extends import_obsidian4.Modal {
  ...
  onOpen() {
    ...
    const field = (label, kind = "input", placeholder2) => {
      const row = content.createDiv({ cls: "tm-editor-field" });
      const caption = row.createEl("label", { text: label });
      const input = kind === "select" ? row.createEl("select") : row.createEl("input", { type: "text", attr: { placeholder: placeholder2 != null ? placeholder2 : "" } });
...
var SmartListEditorModal = class extends import_obsidian5.Modal {
  ...
  onOpen() {
    ...
    const field = (label, select = false) => {
      const row = content.createDiv({ cls: "tm-editor-field" });
      const caption = row.createEl("label", { text: label });
    ...
    const submit = async () => {
      ...
      if (!draft.name) {
        error.setText("Enter a list name.");
        name.focus();
        return;
      }
      draft.sort = sort.value;
      draft.descending = direction.value === "descending";
      draft.grouping = grouping.value;
...
// src/task-line-editor.ts
var import_commands = require("@codemirror/commands");
var import_state2 = require("@codemirror/state");
var import_view2 = require("@codemirror/view");

// src/task-row-details.ts
var import_obsidian6 = require("obsidian");
function taskDayDistance(date, now2 = /* @__PURE__ */ new Date()) {
  const [year, month, day] = date.split("-").map(Number);
  return Math.round((Date.UTC(year, month - 1, day) - Date.UTC(now2.getFullYear(), now2.getMonth(), now2.getDate())) / 864e5);
}
function taskScheduleLabel(date, now2 = /* @__PURE__ */ new Date()) {
...
function taskDeadlineLabel(date, now2 = /* @__PURE__ */ new Date()) {
  const days = taskDayDistance(date, now2);
  if (days === 0) return "Today";
...
function deadlineIsOverdue(date, time, now2 = /* @__PURE__ */ new Date()) {
  ...
  return hour * 60 + minute < now2.getHours() * 60 + now2.getMinutes();
}
function taskTimeLabel(time) {
  const [hour, minute] = time.split(":").map(Number);
...
function renderTaskDetails(primary, metadata, task, options) {
  var _a, _b, _c;
  const now2 = (_a = options.now) != null ? _a : /* @__PURE__ */ new Date();
  ...
  const showDate = (field) => show(field) && grouping !== field && !(grouping === "date" && task[field] === actionDate2);
  ...
  if (due || dueTime) {
    ...
    (0, import_obsidian6.setIcon)(icon, "flag");
    badge.createSpan({ text: [due, dueTime].filter(Boolean).join(", ") });
    editable(badge, `Edit deadline: ${(_c = task.deadline) != null ? _c : ""}${dueTime ? `, ${dueTime}` : ""}`, () => options.edit("deadline"));
  }
  ...
  if (show("tags") && grouping !== "tags") for (const tag of options.tags) {
    ...
    (0, import_obsidian6.setIcon)(icon, "tag");
...
var moment2 = import_obsidian7.moment;
...
// src/task-line-editor.ts
function inlineTaskTokens(text, dateFormat) {
  var _a, _b;
  const result = [];
  let offset = 0;
  for (const line of text.split("\n")) {
    ...
    const tokens = (_b = (_a = noteTaskPresentation(presentationSource, dateFormat)) == null ? void 0 : _a.tokens) != null ? _b : taskTokens(source, dateFormat);
    ...
    for (const match of line.matchAll(/(?:~)?\[\[([^\]\n]+)\]\]/g)) {
      ...
      const project = match[0].startsWith("~") ? target : void 0;
      const label = project ? target.split("|")[0].split("#")[0].replace(/\.md$/i, "").split("/").pop() : target.includes("|") ? target.slice(target.indexOf("|") + 1) : target.replace(/#/g, " \u203A ");
...
function taskModeEditorText(text, dateFormat) {
  if (text.includes("\n")) return text;
  ...
  const order = (item) => {
    var _a, _b, _c, _d;
    return ((_a = item.token) == null ? void 0 : _a.kind) === "deadline" ? 0 : ((_b = item.token) == null ? void 0 : _b.kind) === "priority" ? 1 : ((_c = item.token) == null ? void 0 : _c.kind) === "scheduledDate" ? 2 : ((_d = item.token) == null ? void 0 : _d.kind) === "durationMinutes" ? 3 : item.project ? 4 : 5;
  };
...
function taskMetadataStart(tokens) {
  var _a;
  return (_a = tokens.find((item) => item.project || item.token && item.token.kind !== "deadline" && item.token.kind !== "priority")) == null ? void 0 : _a.from;
}
var MetadataLineBreak = class extends import_view2.WidgetType {
  eq() {
    return true;
  }
  get lineBreaks() {
  ...
  toDOM(view) {
    return view.dom.ownerDocument.createElement("br");
  }
  ignoreEvent() {
    return false;
  }
};
function inactiveTaskTokens(tokens, selections) {
  return tokens.filter((token) => !selections.some((selection) => selection.from <= token.to && selection.to >= token.from));
}
var InlineTokenWidget = class extends import_view2.WidgetType {
  constructor(item) {
    super();
    this.item = item;
  }
  eq(other) {
    return JSON.stringify(this.item) === JSON.stringify(other.item);
  }
  toDOM(view) {
    var _a, _b, _c;
    ...
    if (token && token.kind !== "priority") {
      ...
      if (token.kind === "scheduledDate") {
        ...
        if (schedule) {
          ...
          date.textContent = (_a = token.dateLabel) != null ? _a : token.label;
      ...
      root.textContent = (_c = (_b = token == null ? void 0 : token.label) != null ? _b : this.item.label) != null ? _c : "";
      if ((token == null ? void 0 : token.kind) === "priority") {
...
var TaskLineEditor = class {
  constructor(parent, value, dateFormat, onChange, prompt = "") {
    this.destroyed = false;
    value = taskModeEditorText(value, dateFormat);
    this.defaultValue = value;
    const decorate = (view) => {
      const tokens = inlineTaskTokens(view.state.doc.toString(), dateFormat);
      const ranges = inactiveTaskTokens(tokens, view.hasFocus ? view.state.selection.ranges : []).map((item) => import_view2.Decoration.replace({ widget: new InlineTokenWidget(item) }).range(item.from, item.to));
      const secondary = taskMetadataStart(tokens);
      if (secondary !== void 0) {
        ranges.push(import_view2.Decoration.widget({ widget: new MetadataLineBreak(), side: -1 }).range(secondary));
        ranges.push(import_view2.Decoration.mark({ class: "tm-editor-secondary" }).range(secondary, view.state.doc.length));
      }
      return import_view2.Decoration.set(ranges, true);
    };
    this.view = new import_view2.EditorView({
      parent,
      state: import_state2.EditorState.create({ doc: value, extensions: [
        import_view2.EditorView.lineWrapping,
        (0, import_view2.placeholder)(prompt),
        import_view2.EditorView.domEventHandlers({ blur: () => {
          queueMicrotask(() => {
        ...
        import_view2.keymap.of([{ key: "Shift-Enter", run: (view) => {
          view.dispatch(view.state.replaceSelection("\n"));
          return true;
        } }, ...import_commands.defaultKeymap.filter((binding) => binding.key !== "Mod-Enter"), ...import_commands.historyKeymap]),
        import_view2.EditorView.contentAttributes.of({ "aria-label": "Task text", "aria-multiline": "true", role: "textbox" }),
...
function bulkPropertyPatch(values, dateFormat, reference = /* @__PURE__ */ new Date()) {
  var _a, _b, _c;
  const patch = {};
  for (const field of ["scheduled", "deadline"]) {
    if (!(field in values)) continue;
    const value = values[field].trim();
    const parsed = value ? parseDateTimeExpression(value, reference, dateFormat) : void 0;
    if (value && !parsed) throw new Error(`Could not understand the ${field === "scheduled" ? "scheduled date and time" : "deadline date and time"}.`);
    if (field === "scheduled") {
      patch.scheduledDate = parsed == null ? void 0 : parsed.date;
      patch.scheduledTime = parsed == null ? void 0 : parsed.time;
    } else {
      patch.deadline = parsed == null ? void 0 : parsed.date;
      patch.deadlineTime = parsed == null ? void 0 : parsed.time;
    }
  }
  ...
  if ("priority" in values) {
    ...
    patch.priority = values.priority ? Number(values.priority) : void 0;
  }
...
var labels = { scheduled: "Scheduled date", deadline: "Deadline", duration: "Duration", priority: "Priority", tags: "Tags", destination: "Project", description: "Description" };
var propertyKeys = ["scheduledDate", "scheduledTime", "deadline", "deadlineTime", "durationMinutes", "priority", "tags", "destination"];
function commonBulkValues(tasks, dateFormat) {
...
function bulkInlineText(values, dateFormat) {
  var _a;
  const properties = bulkPropertyPatch(values, dateFormat);
  const draft = { ...properties, title: "", completed: false, indent: 0, destination: (_a = properties.destination) != null ? _a : "" };
  return (draft.destination ? serializeTaskInput(draft, dateFormat) : serializeTask(draft, dateFormat)).replace(/^- \[ \]\s*/, "");
}
function bulkInlinePatch(text, initial, dateFormat, inboxPath) {
  var _a, _b;
  const parse2 = (value) => {
    if (/[\r\n]/.test(value)) throw new Error("Enter task properties on one line.");
    const parsed = parseTaskInput(`Properties ${value}`, /* @__PURE__ */ new Date(), dateFormat);
    if (!parsed || parsed.title !== "Properties") throw new Error("Use task property syntax for dates, duration, priority, tags, and project.");
    return parsed;
  };
  const before = parse2(initial);
  const after = parse2(text);
  const patch = {};
  for (const key of propertyKeys) {
    const previous = key === "tags" ? (_a = before.tags) != null ? _a : [] : before[key];
    const next = key === "tags" ? (_b = after.tags) != null ? _b : [] : after[key];
    if (JSON.stringify(previous) !== JSON.stringify(next)) Object.assign(patch, { [key]: key === "destination" ? next != null ? next : inboxPath : next });
  }
  if ("scheduledDate" in patch || "scheduledTime" in patch) {
    patch.scheduledDate = after.scheduledDate;
    patch.scheduledTime = after.scheduledTime;
  }
  if ("deadline" in patch || "deadlineTime" in patch) {
    patch.deadline = after.deadline;
    patch.deadlineTime = after.deadlineTime;
  }
  return patch;
}
var BulkTaskEditorModal = class extends import_obsidian9.Modal {
  constructor(app, options) {
    super(app);
    this.options = options;
    this.initialText = "";
  ...
  onOpen() {
    ...
    (0, import_obsidian9.setIcon)(save, "check");
    ...
    this.handleKeydown = (event) => {
      ...
      if (((_a = event.target) == null ? void 0 : _a.tagName) === "BUTTON") return;
      event.preventDefault();
      event.stopPropagation();
    ...
    this.focusTimer = window.setTimeout(() => {
      var _a, _b;
      this.editor.focus();
      const token = this.options.focusProperty ? inlineTaskTokens(this.editor.value, this.options.dateFormat).find((item) => {
        var _a2;
        return ((_a2 = item.token) == null ? void 0 : _a2.kind) === this.options.focusProperty;
      }) : void 0;
      const start = (_a = token == null ? void 0 : token.from) != null ? _a : this.editor.value.length;
      this.editor.setSelectionRange(start, (_b = token == null ? void 0 : token.to) != null ? _b : start);
    }, 0);
  }
  onClose() {
    var _a, _b;
...
function editable2(element, label, field, edit) {
  element.setAttribute("role", "button");
  element.setAttribute("tabindex", "0");
...
function renderProjectDeadline(parent, project, edit, dateFormat, now2 = /* @__PURE__ */ new Date()) {
  if (project.deadline) {
    ...
    (0, import_obsidian10.setIcon)(due.createSpan({ cls: "tm-task-detail-icon", attr: { "aria-hidden": "true" } }), "flag");
...
function renderDashboard(container, options) {
  ...
  const panel = (row, title, name, render, add) => {
    ...
    if (add) {
      ...
      (0, import_obsidian11.setIcon)(button, "plus");
...
function renderDescriptionIndicator(parent, description) {
  ...
  (0, import_obsidian12.setIcon)(icon, "align-left");
}

// src/task-selection.ts
var TaskSelection = class {
  constructor() {
  ...
  has(task) {
    var _a;
    return ((_a = this.selected.get(task.id)) == null ? void 0 : _a.raw) === task.raw;
  }
  tasks(visible) {
    return visible.filter((task) => this.has(task));
  }
  retain(visible) {
    var _a, _b;
    const current = new Map(visible.map((task) => [task.id, task]));
    for (const [id, task] of this.selected) if (((_a = current.get(id)) == null ? void 0 : _a.raw) !== task.raw) this.selected.delete(id);
    if (!this.selected.has((_b = this.anchor) != null ? _b : "")) this.anchor = void 0;
  }
  click(task, visible, shift = false, additive = false) {
...
function updateProjectDate(frontmatter, field, value, expected, dateFormat) {
  var _a, _b, _c;
  ...
  const key = (_b = (_a = matches.find((key2) => frontmatter[key2] !== null && frontmatter[key2] !== void 0)) != null ? _a : matches[0]) != null ? _b : field === "scheduledDate" ? "date" : field === "endDate" ? "end date" : "deadline";
...
function updateProjectDates(frontmatter, changes, expected, dateFormat) {
  ...
  for (const field of ["scheduledDate", "endDate", "deadline"]) {
...
function renderGantt(container, options) {
  ...
  for (const [value, label] of [["month", "M"], ["quarter", "Q"], ["year", "Y"], ["five-year", "5Y"]]) {
  ...
  const syncViewport = () => {
    var _a, _b, _c;
    const labelWidth = (_b = (_a = header.firstElementChild) == null ? void 0 : _a.getBoundingClientRect().width) != null ? _b : 330;
  ...
  const persist = async (project, changes, rebuild = false) => {
    ...
    try {
      ...
      new import_obsidian13.Notice(cause instanceof Error ? cause.message : "Could not update project dates.");
    } finally {
      busy = false;
...
function taskMatchesQuery(task, query, inboxPath, now2 = /* @__PURE__ */ new Date()) {
  var _a, _b, _c, _d, _e;
  if (query.tag !== void 0 && !((_a = task.tags) == null ? void 0 : _a.includes(query.tag))) return false;
  if ((_b = query.filters) == null ? void 0 : _b.some((filter) => !matchesFilter(task, filter))) return false;
  if (!query.showCompleted && !((_c = query.filters) == null ? void 0 : _c.some((filter) => filter.property === "status")) && task.completed) return false;
...
var ListDragController = class {
  ...
  row(row, primary, task, group) {
    ...
    row.addEventListener("pointerdown", (event) => {
      if (event.button !== 0 || import_obsidian14.Platform.isMacOS && event.ctrlKey || this.busy) return;
...
function renderCalendar(container, options) {
  ...
  for (const [delta, icon, label] of [[-1, "chevron-left", "Previous period"], [1, "chevron-right", "Next period"]]) {
    const button = controls.createEl("button", { cls: "clickable-icon", attr: { "aria-label": label, title: label } });
    (0, import_obsidian15.setIcon)(button, icon);
  ...
  if (planner) {
    ...
    (0, import_obsidian15.setIcon)(icon, "panel-right");
  ...
  const dropTarget = (element, targetDate, getTime) => {
    ...
    element.addEventListener("drop", (event) => {
      ...
      void options.move(task, targetDate, getTime == null ? void 0 : getTime(event)).catch((cause) => {
        new import_obsidian15.Notice(cause instanceof Error ? cause.message : "Could not reschedule task.");
      }).finally(() => {
        moving = false;
      });
    });
  ...
  const taskCard = (parent, task) => {
    ...
    checkbox.addEventListener("change", () => {
      ...
      void ((_a3 = options.toggle) == null ? void 0 : _a3.call(options, task, checkbox.checked).catch((cause) => {
        checkbox.checked = task.completed;
        new import_obsidian15.Notice(cause instanceof Error ? cause.message : "Could not complete task.");
      }).finally(() => {
    ...
    if (task.deadline && calendarDate(task) === task.deadline && (time != null ? time : "") === ((_a2 = task.deadlineTime) != null ? _a2 : "")) {
      (0, import_obsidian15.setIcon)(card.createSpan({ cls: "tm-calendar-task-flag", attr: { "aria-label": "Deadline" } }), "flag");
    }
    renderDescriptionIndicator(title2, task.description);
    card.draggable = true;
    card.addEventListener("click", (event) => {
      if (!options.bind || event.target.closest(".tm-calendar-task-title")) {
        event.stopPropagation();
        options.edit(task);
      }
    });
      ...
      for (const edge of ["start", "end"]) {
        ...
        const save = () => {
          ...
          void options.resize(task, day, minuteTime(range.start), range.duration).catch((cause) => {
            restore();
            new import_obsidian15.Notice(cause instanceof Error ? cause.message : "Could not resize task.");
          }).finally(() => {
            moving = false;
...
var TaskMainView = class extends import_obsidian16.ItemView {
  ...
  getDisplayText() {
    var _a, _b, _c;
    if (this.state.mode === "smartLists" && this.state.smartListId) return (_b = (_a = this.plugin.settings.smartLists.find((list) => list.id === this.state.smartListId)) == null ? void 0 : _a.name) != null ? _b : "Smart list not found";
    if (this.state.mode === "tags" && this.state.tag) return this.state.tag;
    if (this.pagePath) return (_c = this.pagePath.replace(/\.md$/i, "").split("/").pop()) != null ? _c : "Project";
  ...
  async setState(state) {
    ...
    this.state.tag = typeof state.tag === "string" && state.tag ? state.tag : void 0;
  ...
  renderHeader(container) {
    ...
    if (this.state.mode === "tags" && this.state.tag) {
      ...
      (0, import_obsidian16.setIcon)(back, "arrow-left");
    ...
    for (const [layout, icon2, label] of [["list", "list", "List"], ["calendar", "calendar-days", "Calendar"], ["kanban", "columns-3", "Kanban"]]) {
      ...
      (0, import_obsidian16.setIcon)(button, icon2);
    ...
    (0, import_obsidian16.setIcon)(icon, "plus");
  ...
  renderFilters(container, toggle) {
    ...
    const sync = () => {
      (0, import_obsidian16.setIcon)(toggle, "sliders-vertical");
    ...
    const selectOption = (label, value, choices, change) => {
    ...
    selectOption("Sort by", this.sort, [{ key: "date", label: "Action date and time" }, ...TASK_PROPERTIES.filter((property) => property.key !== "scheduledTime" && property.key !== "deadlineTime").map((property) => ({ ...property, label: property.key === "scheduledDate" ? "Scheduled date and time" : property.key === "deadline" ? "Deadline date and time" : property.label }))], (value) => {
      this.sort = value;
    });
    selectOption("Sort direction", this.descending ? "descending" : "ascending", [
      ...
      this.descending = value === "descending";
    });
    selectOption("Group by", this.grouping, [
      { key: "default", label: "View default" },
      { key: "none", label: "None" },
  ...
  renderSmartLists(container) {
    const header = container.createDiv({ cls: "tm-view-header" });
    header.createEl("h1", { text: "Smart Lists" });
    header.createEl("button", { text: "Create new smart list", cls: "mod-cta" }).addEventListener("click", () => this.plugin.openSmartListEditor());
    ...
    for (const list of lists) {
      ...
      (0, import_obsidian16.setIcon)(icon, "list-filter");
  ...
  renderTagList(container) {
    ...
    const render = () => {
      ...
      for (const tag of tags) {
        ...
        const icon = row.createSpan({ cls: "tm-project-icon" });
        (0, import_obsidian16.setIcon)(icon, "tag");
        const content = row.createDiv({ cls: "tm-task-content" });
        const title = content.createEl("button", { cls: "tm-task-title", text: tag.name });
        title.addEventListener("click", () => void this.plugin.openTag(tag.name).catch((error) => new import_obsidian16.Notice(String(error))));
  ...
  renderProjectList(container) {
    ...
    (0, import_obsidian16.setIcon)(create, "plus");
    ...
    if (!projects.length) {
      ...
      (0, import_obsidian16.setIcon)(icon, "target");
    ...
    if (this.projectLayout === "gantt") {
      renderGantt(container, {
        ...
        open: (project) => {
          void this.plugin.openProject(project.path).catch((error) => new import_obsidian16.Notice(String(error)));
        },
        ...
        update: async (project, changes) => {
          ...
          if (!(file instanceof import_obsidian16.TFile)) throw new Error("Project note no longer exists.");
  ...
  renderProjectGroup(container, title, projects) {
    ...
    for (const { project, depth } of projectHierarchy(projects)) {
      ...
      button.addEventListener("click", () => void this.plugin.openProject(project.path).catch((error) => new import_obsidian16.Notice(String(error))));
  ...
  async dropListTask(original, group, originalAnchor, placement) {
    try {
      ...
      if (this.layout === "kanban" && group && selected.some((task) => {
        ...
        return group.value !== (previous == null ? void 0 : previous.value) || group.destination && group.destination !== draftForGroup(task).destination;
      })) {
      ...
      new import_obsidian16.Notice(cause instanceof Error ? cause.message : "Could not move selected tasks.");
    } finally {
      this.draggedTasks = [];
    }
  }
  getSelectedTasks() {
    return this.selection.tasks(this.visibleTasks);
  }
  clearSelection() {
    this.selection.clear();
    this.updateSelection();
  ...
  clearSelectionOutside(event) {
    if (event.button !== 0 || import_obsidian16.Platform.isMacOS && event.ctrlKey || !this.getSelectedTasks().length) return;
    const target = event.target;
  ...
  bindSelection(row, task) {
    ...
    const rows = (_a = this.selectionRows.get(task.id)) != null ? _a : [];
    rows.push(row);
    ...
    const selectForContextMenu = (event) => {
      const additive = import_obsidian16.Platform.isMacOS ? event.metaKey : event.ctrlKey;
    ...
    row.addEventListener("pointerdown", (event) => {
      this.contextSelectionOnPress = event.button === 2 || import_obsidian16.Platform.isMacOS && event.button === 0 && event.ctrlKey;
    ...
    row.addEventListener("click", (event) => {
      if ((import_obsidian16.Platform.isMacOS ? event.metaKey : event.ctrlKey) && this.selection.has(task)) {
        event.preventDefault();
        event.stopImmediatePropagation();
  ...
  renderTaskRow(list, task, depth, target) {
    ...
    const toggleTask = async () => {
      ...
      try {
        ...
        checkbox.disabled = false;
        new import_obsidian16.Notice(cause instanceof Error ? cause.message : "Could not update the task.");
      }
    };
    checkbox.addEventListener("change", () => {
      void toggleTask();
    ...
    try {
      if (recurringFile(this.app, task)) {
        ...
        (0, import_obsidian16.setIcon)(icon, "repeat-2");
      }
    } catch (e) {
    }
    ...
    const tags = ((_c = task.tags) != null ? _c : []).filter((tag) => {
      var _a2;
      if (this.state.mode !== "tags") return true;
      return this.pagePath ? ((_a2 = this.app.metadataCache.getFirstLinkpathDest(tag, task.path)) == null ? void 0 : _a2.path) !== this.pagePath : tag !== this.state.tag;
    });
    renderTaskDetails(primary, metadata, task, {
      grouping: this.metadataGrouping,
      show: (property) => this.showTaskProperty(property),
      dateFormat: this.plugin.dateFormat(),
      source: task.path !== implicitSource ? task.path : void 0,
      tags,
      edit: (property) => this.editTask(task, property),
      openSource: () => {
  ...
  renderEmpty(container) {
    const empty = container.createDiv({ cls: "tm-empty" });
    const icon = empty.createDiv({ cls: "tm-empty-icon" });
    (0, import_obsidian16.setIcon)(icon, "circle-check-big");
  ...
  async drain() {
    while (this.pending && !this.disposed) {
      ...
      for (const leaf of leaves) {
        ...
        if (view instanceof import_obsidian17.MarkdownView && view.file && this.enabled() && (this.isProject(view.file.path) || this.tagForPath(view.file.path))) {
...
function noteDateInput(getDateFormat, isTaskMode, getLinkDates = () => true) {
  return import_state3.EditorState.transactionFilter.of((transaction) => {
...
var DateLabelWidget = class extends import_view3.WidgetType {
  ...
  toDOM(view) {
    ...
    if (this.linkText) {
      ...
      const open = (event) => {
        ...
        const info = view.state.field(import_obsidian18.editorInfoField, false);
        if (!info) return;
        event.preventDefault();
        event.stopPropagation();
        void info.app.workspace.openLinkText(this.linkText, (_b = (_a = info.file) == null ? void 0 : _a.path) != null ? _b : "", event.button === 1 || (import_obsidian18.Platform.isMacOS ? event.metaKey : event.ctrlKey));
      };
...
var NoteTaskDetailsWidget = class extends import_view3.WidgetType {
...
function noteTaskDecorations(tasks, selections) {
  ...
  const decorations = [];
  for (const task of tasks) {
    decorations.push(import_view3.Decoration.line({ attributes: { class: "tm-note-task-line", "data-tm-priority": String((_a = task.presentation.priority) != null ? _a : "") } }).range(task.lineFrom));
    if (selections.some((selection) => selection.from <= task.lineTo && selection.to >= task.lineFrom)) continue;
    decorations.push(import_view3.Decoration.replace({ widget: new NoteTaskDetailsWidget(task.presentation, task.from) }).range(task.from, task.to));
  }
  return decorations;
}
function noteTokenMarks(tokens, viewport, selections) {
  ...
  for (const { from, to, token } of tokens) {
    ...
    pills.push(import_view3.Decoration.mark({
      class: `${tokenClass(token)} tm-note-token-editor`,
      attributes: { title: token.description, style: notePropertyIconStyle(token.kind) }
    }).range(from, to));
    if (token.display) {
      syntax.push(import_view3.Decoration.replace({ widget: new DateLabelWidget(token.display.label, token.display.linkText) }).range(from + token.display.from - token.from, from + token.display.to - token.from));
    }
    if (token.kind === "tags") {
      syntax.push(import_view3.Decoration.mark({ class: "tm-note-token-tag-prefix" }).range(from, from + 1));
    }
    if (token.kind === "deadline") {
      syntax.push(import_view3.Decoration.mark({ class: "tm-note-token-brace" }).range(from, from + 1));
      syntax.push(import_view3.Decoration.mark({ class: "tm-note-token-brace" }).range(to - 1, to));
    }
  }
  return { pills: import_view3.Decoration.set(pills, true), syntax: import_view3.Decoration.set(syntax, true) };
}
function noteTokenEditor(getDateFormat) {
  return import_view3.ViewPlugin.fromClass(class {
    ...
    provide: (plugin) => import_view3.EditorView.outerDecorations.of((view) => {
      var _a, _b;
      return (_b = (_a = view.plugin(plugin)) == null ? void 0 : _a.pills) != null ? _b : import_view3.Decoration.none;
    })
  });
}
...
function handleTaskEditClick(event, resolve, open) {
  var _a;
  if (!(import_obsidian19.Platform.isMacOS ? event.metaKey : event.ctrlKey) || event.button !== 0) return;
  const target = event.target;
...
function handleRecurringTaskClick(event, resolve, complete2) {
  var _a, _b;
  if (!complete2 || event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.altKey || event.shiftKey) return;
  const checkbox = (_b = (_a = event.target) == null ? void 0 : _a.closest) == null ? void 0 : _b.call(_a, 'input[type="checkbox"]');
  if (!checkbox) return;
  const task = resolve(checkbox);
  if (!task || task.completed || !complete2(task)) return;
  event.preventDefault();
  event.stopImmediatePropagation();
}
function bindNoteTaskEdit(root, resolve, open, complete2) {
  ...
  const click = (event) => {
    ...
    handleRecurringTaskClick(event, resolve, complete2);
  };
...
function noteTaskEditEditor(getDateFormat, open, getSectionHeadingLevel = () => 1, complete2) {
...
function registerNoteTaskEdit(root, context, getDateFormat, open, getSectionHeadingLevel = () => 1, complete2) {
  const child = new import_obsidian19.MarkdownRenderChild(root);
...
var TaskEditorModal = class extends import_obsidian20.Modal {
  constructor(app, options) {
    super(app);
    this.options = options;
    this.taskIndent = "";
  ...
  onOpen() {
    ...
    const initial = this.options.task ? this.options.task.raw + this.serializeDraft(this.draft).slice(serializeTask(this.draft, this.options.dateFormat, this.options.settings.linkDates).length) : this.serializeDraft(this.draft);
    ...
    const updatePriority = () => {
      var _a;
      const parsed = parseTaskInput(this.rawInput.value, /* @__PURE__ */ new Date(), this.options.dateFormat);
      taskLine.setAttribute("data-tm-priority", String((_a = parsed == null ? void 0 : parsed.priority) != null ? _a : ""));
      error.empty();
    };
    ...
    const deleteButton = this.options.task && this.options.onDelete ? actions.createEl("button", { cls: "tm-delete-task tm-editor-icon-action", attr: { "aria-label": "Delete task", title: this.options.task.childIds.length ? "Delete this task and its subtasks" : "Delete this task" } }) : void 0;
    if (deleteButton) (0, import_obsidian20.setIcon)(deleteButton, "trash-2");
    const save = actions.createEl("button", { cls: "mod-cta tm-editor-icon-action", attr: { "aria-label": "Save task", title: "Save task" } });
    (0, import_obsidian20.setIcon)(save, "check");
    ...
    this.focusTimer = window.setTimeout(() => {
      ...
      if (this.options.focusProperty) {
        const ranges = [];
        parseTaskLine(`- [ ] ${this.rawInput.value}`, /* @__PURE__ */ new Date(), this.options.dateFormat, false, ranges);
  ...
  readRaw() {
    ...
    if (!this.options.task) return parseTaskTreeInput(markdown, this.options.settings.inboxPath, /* @__PURE__ */ new Date(), this.options.dateFormat, this.options.settings.linkDates);
    ...
    const parsed = parseTaskInput(value, /* @__PURE__ */ new Date(), this.options.dateFormat, true);
    if (!(parsed == null ? void 0 : parsed.title)) throw new Error("Enter a task title.");
...
var TaskIndex = class {
  ...
  async refreshPath(path) {
    ...
    if (file instanceof import_obsidian21.TFile) await this.refreshFile(file);
  }
  ...
  updateProjectStatus(file) {
    ...
    const tags = cache ? (0, import_obsidian21.getAllTags)(cache) : null;
...
var TaskNavigationView = class extends import_obsidian22.ItemView {
  ...
  async onOpen() {
    ...
    const syncActive = () => {
      var _a;
      const view = this.app.workspace.getActiveViewOfType(import_obsidian22.ItemView);
  ...
  setActive(mode, tag, project, smartListId) {
  ...
  render() {
    ...
    const action = (label, icon, run) => {
      ...
      button.addEventListener("click", () => {
        void Promise.resolve().then(run).catch((error) => new import_obsidian22.Notice(String(error)));
      });
      return button;
    };
    ...
    const toggle = action("Task mode", "list-checks", async () => {
      ...
      try {
        await this.plugin.setTaskMode(!this.plugin.settings.taskMode);
      } finally {
        this.render();
      }
    });
    ...
    const item = (parent, label, active, open) => {
      ...
      button.addEventListener("click", () => {
        void open().catch((error) => new import_obsidian22.Notice(String(error)));
      });
      return row;
    };
    for (const entry of NAV_ITEMS) {
      ...
      const row = item(
        group,
        entry.label,
        entry.mode === this.activeMode && !this.activeTag && !this.activeProject && !this.activeSmartList,
      ...
      (0, import_obsidian22.setIcon)(collapse, expanded ? "chevron-down" : "chevron-right");
      row.prepend(collapse);
      collapse.addEventListener("click", () => {
        if (expanded) this.expanded.delete(entry.mode);
        else this.expanded.add(entry.mode);
        this.render();
      });
...
function planBulkTasks(contents, changes, options = {}) {
  ...
  const contains = (parent, child) => parent !== child && parent.task.path === child.task.path && parent.block.start <= child.block.start && parent.block.end > child.block.start;
  ...
  const anchor = options.anchor ? liveTaskBlock(get(options.anchor.path), options.anchor, options.dateFormat) : void 0;
...
var TaskStore = class {
  ...
  async update(task, draft) {
    ...
    if ((0, import_obsidian23.normalizePath)(destination.path) !== task.path || destination.heading !== task.section) {
  ...
  async bulkUpdate(tasks, patch) {
    if (!Object.keys(patch).length) return [];
    return this.bulkChange(tasks, (task) => ({ ...draftForGroup(task), ...patch }));
  }
  async bulkDelete(tasks) {
    return this.bulkChange(tasks, () => void 0, { delete: true });
  }
  async bulkDrop(tasks, group, anchor, placement) {
    return this.bulkChange(tasks, (task) => draftForGroup(task, group), { anchor, placement });
  }
  async bulkChange(tasks, draft, options = {}) {
  ...
  requireFile(path) {
    const file = this.app.vault.getAbstractFileByPath((0, import_obsidian23.normalizePath)(path));
    if (!(file instanceof import_obsidian23.TFile)) throw new Error(`Cannot find note: ${path}`);
    return file;
  }
  async ensureFile(path) {
    const normalized = (0, import_obsidian23.normalizePath)(path.endsWith(".md") ? path : `${path}.md`);
    const existing = this.app.vault.getAbstractFileByPath(normalized);
    if (existing instanceof import_obsidian23.TFile) return existing;
    ...
    for (const part of parts) {
      ...
      else if (!(folder instanceof import_obsidian23.TFolder)) throw new Error(`${current} is not a folder.`);
    }
...
var TaskManagerSettingTab = class extends import_obsidian24.PluginSettingTab {
        ...
        render: (setting) => {
          setting.addButton((button) => button.setButtonText("Update dates").onClick(async () => {
            ...
            try {
              ...
              new import_obsidian24.Notice(String(error));
            } finally {
  ...
  display() {
    ...
    for (const group of this.getSettingDefinitions()) {
      ...
      new import_obsidian24.Setting(section).setName(group.heading).setHeading();
      for (const definition of group.items) {
        const setting = new import_obsidian24.Setting(section).setName(definition.name).setDesc(definition.desc);
...
// src/main.ts
var TaskManagerPlugin = class extends import_obsidian25.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    this.registerEditorExtension(noteRecurringCompletion(() => this.dateFormat(), (task) => {
      ...
      try {
        return Boolean(recurringFile(this.app, task));
      } catch (e) {
        return false;
      }
    }, (task) => {
      this.completeRecurringTaskFromNote(task);
    }));
    ...
    this.addRibbonIcon("circle-check-big", "Open task manager", () => void this.activateNavigation().catch((error) => new import_obsidian25.Notice(String(error))));
    const commands = [
    ...
    for (const [mode, name, id] of commands) {
      this.addCommand({ id, name, callback: () => void this.openTaskView({ mode }).catch((error) => new import_obsidian25.Notice(String(error))) });
    }
      ...
      for (const layout of layouts) {
        this.addCommand({
          ...
          checkCallback: (checking) => {
            ...
            if (!checking) {
              void view.setState({ ...state, [isProjects ? "projectLayout" : "layout"]: layout }).then(() => this.app.workspace.requestSaveLayout()).catch((error) => new import_obsidian25.Notice(String(error)));
            }
            return true;
          }
        });
    ...
    this.addCommand({ id: "delete-smart-list", name: "Delete smart list", checkCallback: (checking) => {
      const list = this.activeSmartList();
      if (!list) return false;
      if (!checking) void this.deleteSmartList(list.id).catch((error) => new import_obsidian25.Notice(String(error)));
      return true;
    } });
    this.addCommand({ id: "edit-project", name: "Edit project", checkCallback: (checking) => {
      var _a, _b;
      const view = (_a = this.app.workspace.getActiveViewOfType(TaskMainView)) != null ? _a : this.app.workspace.getActiveViewOfType(import_obsidian25.MarkdownView);
      const path = view instanceof TaskMainView ? view.pagePath : view instanceof import_obsidian25.MarkdownView ? (_b = view.file) == null ? void 0 : _b.path : void 0;
    ...
    this.addCommand({ id: "toggle-task-mode", name: "Toggle task mode", callback: () => {
      void this.setTaskMode(!this.settings.taskMode).catch((error) => new import_obsidian25.Notice(String(error)));
    } });
    ...
    this.addCommand({
      ...
      checkCallback: (checking) => {
        ...
        if (!(file instanceof import_obsidian25.TFile) || file.extension !== "md") return false;
  ...
  async openTaskView(state) {
    ...
    for (const navLeaf of this.app.workspace.getLeavesOfType(TASK_NAV_VIEW)) {
      ...
      if (view instanceof TaskNavigationView) view.setActive(state.mode, state.tag, void 0, state.smartListId);
    }
  }
  activeSmartList() {
    var _a;
    const state = (_a = this.app.workspace.getActiveViewOfType(TaskMainView)) == null ? void 0 : _a.getState();
    return (state == null ? void 0 : state.mode) === "smartLists" ? this.settings.smartLists.find((list) => list.id === state.smartListId) : void 0;
  }
  openSmartListEditor(list) {
    new SmartListEditorModal(this.app, this.index.allTasks(), async (draft) => {
      const saved = await this.saveSmartList(draft, list == null ? void 0 : list.id);
      await this.openTaskView({ mode: "smartLists", smartListId: saved.id });
    }, list).open();
  }
  async saveSmartList(draft, id) {
    ...
    try {
      ...
      this.settings.smartLists = previous;
      throw cause;
    }
  ...
  openProjectEditor(path, focusProperty) {
    ...
    if (!(file instanceof import_obsidian25.TFile) || !project) {
      new import_obsidian25.Notice("Project note no longer exists.");
      return;
    }
  ...
  async openProject(path) {
    var _a;
    const file = this.app.vault.getAbstractFileByPath(path);
    if (!(file instanceof import_obsidian25.TFile)) throw new Error("Project note no longer exists.");
  ...
  completeRecurringTaskFromNote(task) {
    ...
    this.recurringNoteQueue = this.recurringNoteQueue.then(async () => {
      var _a;
      const markdown = this.app.workspace.getActiveViewOfType(import_obsidian25.MarkdownView);
      if (((_a = markdown == null ? void 0 : markdown.file) == null ? void 0 : _a.path) === task.path) await markdown.save();
      const paths = await this.store.resolveRecurring(task, "COMPLETED");
      for (const path of paths) await this.index.refreshPath(path);
    }).catch((error) => {
      new import_obsidian25.Notice(error instanceof Error ? error.message : "Could not complete recurring task.");
  ...
  recurringTaskCommand(checking, outcome) {
    ...
    try {
      ...
      if (!checking) new import_obsidian25.Notice(String(error));
      return false;
    }
    if (!checking) {
      const selected = task;
      void (async () => {
        ...
        for (const path of paths) await this.index.refreshPath(path);
      })().catch((error) => new import_obsidian25.Notice(error instanceof Error ? error.message : "Could not resolve recurring task."));
    }
    return true;
  }
  ...
  async convertToProject(file) {
    try {
      ...
      await this.index.refreshPath(file.path);
      new import_obsidian25.Notice(`Converted ${file.basename} to a project.`);
    } catch (error) {
      new import_obsidian25.Notice(error instanceof Error ? error.message : "Could not convert the note to a project.");
    }
  }
  openBulkEditor(view, focusProperty) {
    ...
    new BulkTaskEditorModal(this.app, {
      ...
      onSave: async (patch) => {
        await refresh(await this.store.bulkUpdate(tasks, patch));
      },
      onDelete: async () => {
        await refresh(await this.store.bulkDelete(tasks));
      }
    }).open();
  }
  openEditor(state) {
  ...
  async updateTaskDates() {
    ...
    await this.saveSettings();
    await this.refreshDateParsing();
    new import_obsidian25.Notice(`Updated task dates in ${paths.length} note${paths.length === 1 ? "" : "s"}.`);
  }
  dateFormat() {
